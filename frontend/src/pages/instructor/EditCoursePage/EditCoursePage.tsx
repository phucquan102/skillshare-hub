...
const EditCoursePage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourse = async () => {
      ...
      try {
        ...
        // ✅ Call instructor-specific API route
        ...
        // ✅ IMPORTANT FIX: Handle instructor being either object or string
        let instructorId: string;
        
        if (typeof courseData.instructor === 'string') {
          // instructor is string ID
          instructorId = courseData.instructor;
          console.log('📝 Instructor is string:', instructorId);
        } else if (courseData.instructor && (courseData.instructor as any)._id) {
          // instructor is object with _id
          instructorId = (courseData.instructor as any)._id;
        ...
        const errorMessage = error.response?.data?.message || error.message || 'Unable to load course information';
 ...
 const handleSubmit = async (
  ...
  if (!courseId || !course) {
    setError('Course not found for update');
    return;
  }
  ...
  try {
    ...
    // ✅ IMPORTANT FIX: Prepare update payload
    const updateData: EditCourseData = {
      ...
      // ✅ IMPORTANT: Only send schedules if changed, otherwise omit
      schedules: (formData as any).schedules && (formData as any).schedules.length > 0 
        ? (formData as any).schedules 
        : undefined // omit this field if unchanged
    };

    // ✅ Remove undefined fields to avoid server errors
    ...
    if (submitType === 'save') {
      ...
      // Only submit for approval if course is not already approved
      if (course.approvalStatus?.status !== 'approved') {
        await courseService.submitForApproval(courseId);
        alert('Course updated and submitted for admin approval!');
      } else {
    ...
    // Show server error details if provided
  ...
  // Error when course failed to load
  if (error && !course) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2">
                ...
                {course.schedules && course.schedules.length > 0 && (
                  <span className="px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                    📅 {course.schedules.length} schedules
                  </span>
                )}
              </div>
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 text-red-700 p-4 rounded-xl mb-6">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h4 className="font-semibold">Error updating course</h4>
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-sm">
            ...
            <div className="bg-white rounded-lg p-3 border border-blue-100">
              <div className="text-blue-600 font-medium">Approval</div>
              <div className="text-blue-900 mt-1">
                {course.approvalStatus?.status === 'approved'
                  ? '✅ Approved'
                  : course.approvalStatus?.status === 'rejected'
            ...
            <div className="bg-white rounded-lg p-3 border border-blue-100">
              <div className="text-blue-600 font-medium">Students</div>
              <div className="text-blue-900 mt-1">
            ...
            <div className="bg-white rounded-lg p-3 border border-blue-100">
              <div className="text-blue-600 font-medium">Ratings</div>
              <div className="text-blue-900 mt-1">
          ...
          {course.approvalStatus?.reason && (
            <div className="mt-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
              <div className="text-yellow-800 text-sm">
                <strong>Reason from admin:</strong> {course.approvalStatus.reason}
              </div>
            </div>
          )}
        </div>
        ...
        {course.schedules && course.schedules.length > 0 && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-6">
            ...
            <div className="text-sm text-green-800">
              <p className="mb-3">The schedules below will be kept if you don't change them in the form:</p>
                ...
                {course.schedules.map((schedule, index) => (
                  <div key={index} className="bg-white rounded-lg p-3 border border-green-100">
                    <div className="flex items-center justify-between">
                      ...
                      }`} title={schedule.isActive ? 'Active' : 'Disabled'}></div>
                    </div>
                  </div>
                ))}
              </div>
              ...
              <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-blue-700 text-sm">
                  💡 <strong>Note:</strong> If you add new schedules in the form, all old schedules will be replaced. 
                  To keep the current schedules, do not change the schedule section in the form.
                </p>
              </div>
            </div>
          </div>
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-6 bg-gray-100 border border-gray-300 rounded-xl p-4">
            ...
            <details>
              <summary className="cursor-pointer text-sm text-gray-600">View course data</summary>
